...
RUN CGO_ENABLED=0 GOFLAGS="-trimpath" GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o /out/corsa-node ./cmd/corsa-node
RUN CGO_ENABLED=0 GOFLAGS="-trimpath" GOOS=linux GOARCH=amd64 go build -ldflags="-s -w" -o /out/corsa-cli ./cmd/corsa-cli
