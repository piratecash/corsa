...
- `internal/core/storage`: the node's shared SQLite state database — the single owner of the file, its connection pool and its versioned schema (see [storage.md](storage.md)). Opened by the process composition root (`desktop.Run` / `sdk.New`) and handed to repositories as a non-owning executor.
- `internal/core/chatlog`: the chat-history repository inside that shared database (see [chatlog.md](chatlog.md)). Reached through `ChatlogGateway` (service layer), not by `node.Service`; it owns SQL, not the database.
...
- `internal/core/service`: desktop-facing application service layer (see [dm_router.md](dm_router.md) for the DMRouter service layer). `DesktopClient` is the composition root; concrete work is delegated to `AppInfo` (config snapshot), `LocalRPCClient` (frame dispatch), `ChatlogGateway` (holds `chatlog.Store`), `MessageStoreAdapter` (satisfies `node.MessageStore`), `DMCrypto` (encrypt/decrypt/send/sync), and `NodeProber` (probe + read fetches + routing snapshot). New callers should depend on the narrowest sub-service instead of the full `DesktopClient` surface.
...
- `internal/core/storage`: общая SQLite-база состояния ноды — единственный владелец файла, пула соединений и версионируемой схемы (см. [storage.md](storage.md)). Открывается composition root-ом процесса (`desktop.Run` / `sdk.New`) и передаётся repositories как невладеющий executor.
- `internal/core/chatlog`: repository истории сообщений внутри этой общей базы (см. [chatlog.md](chatlog.md)). Доступ через `ChatlogGateway` (сервисный слой), а не через `node.Service`; он владеет SQL, а не базой.
