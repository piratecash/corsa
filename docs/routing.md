...
**Снятие бана.** Успешный outbound-handshake (`markPeerConnected`) сбрасывает `BannedUntil` — если peer завершил handshake, он совместим. RPC-команда `add_peer` также сбрасывает `BannedUntil` для ручного override оператором. Если peer уже известен, `add_peer` перемещает его в начало очереди дозвона и отвечает «already known, moved to front» вместо добавления дубликата. Сетевые announce-сообщения `BannedUntil` не очищают: сторонний gossip может добавить адрес в кандидаты, но совместимость подтверждается только прямым handshake.
