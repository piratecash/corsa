...
- [Таблица маршрутов](#iter-1)
  ...
  - [Незавершённые задачи Фазы 1](#незавершённые-задачи-фазы-1)
- [Удаление сообщений](#iter-6)
- [Google WSS fallback](#iter-10)
- [Фоновая доставка на Android](#iter-25)
...
в разделе [Незавершённые задачи Фазы 1](#незавершённые-задачи-фазы-1).
Health, RTT и targeted route query отгружены под своим контрактом — см. `docs/routing.md` «Здоровье маршрутов и composite scoring» и wire-протокол в `docs/protocol/route_health.md`.
...
<a id="iter-25"></a>
### Фоновая доставка на Android

**Цель:** надёжная доставка сообщений Android-клиенту, когда приложение в
фоне или закрыто.

**Проблема:** сейчас клиент работает только на переднем плане. Android-
обёртка пока не содержит service и push receiver; Doze/App
Standby ограничивают сеть после ухода Activity в фон, а процесс может быть
уничтожен. В протоколе нет durable mailbox, а retry отправителя сдаётся
примерно через 3,5 часа (`internal/core/node/delivery_retry.go`) — Android-
клиент, закрытый дольше, теряет такие сообщения навсегда.

**Подход:** добавить foreground service в стандартную Android-обёртку, чтобы
удерживать ноду живой в активной сессии, плюс durable-mailbox механизм на
стороне протокола для длинных офлайн-окон.

**Готово когда:** сообщение, отправленное Android-устройству, которое было
офлайн сутки, доставляется при следующем запуске приложения без ручного
resync.
...
```
*Диаграмма — Зависимости итераций и инкрементальная поставка (iter-2 поглощена Phase 3 cluster-mesh плана — см. `docs/cluster-mesh-architecture-plan.md` §10.5)*
