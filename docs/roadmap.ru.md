...
`internal/core/routing/` (модуль маршрутизации),
...
- [Таблица маршрутов](#iter-1)
  - [Выделение сетевого ядра](#выделение-сетевого-ядра-peerconn--netcore)
...
- [Удаление сообщений](#iter-6)
- [Android-приложение](#iter-7)
- [Google WSS fallback](#iter-10)
- [Голосовые звонки](#iter-20)
- [Надёжность, репутация, multi-path и инкрементальная синхронизация](#iter-2)
- [Оптимизация и масштабирование](#iter-3)
- [Структурированный overlay (DHT)](#iter-4)
- [Второй слой шифрования](#iter-8)
- [Обход DPI](#iter-9)
- [SOCKS5-туннель](#iter-11)
- [Групповые чаты](#iter-12)
- [Onion DM](#iter-13)
  - [Onion-обёртка DM envelope](#iter-13-1)
  - [Onion-квитанции](#iter-13-2)
  - [Эфемерные ключи и прямая секретность](#iter-13-3)
  - [Выбор пути и ротация цепочек](#iter-13-4)
  - [Защита от анализа трафика](#iter-13-5)
- [Глобальные имена](#iter-14)
- [Расширения Gazeta](#iter-15)
- [iOS-приложение](#iter-16)
- [BLE last mile](#iter-17)
  - [BLE-транспорт и обнаружение пиров](#iter-17-1)
  - [BLE-фрагментация и адаптация к MTU](#iter-17-2)
  - [BLE-дедупликация и relay](#iter-17-3)
  - [BLE rate limiting и QoS](#iter-17-4)
- [Meshtastic last mile](#iter-18)
  - [Meshtastic transport bridge](#iter-18-1)
  - [Radio-aware фрагментация и pacing](#iter-18-2)
  - [Radio mesh relay и дедупликация](#iter-18-3)
- [Конструктор шифрования](#iter-19)
- [Обнаружение в локальной сети](#iter-22)
- [NAT traversal и hole punching](#iter-23)
- [Управление перегрузками и QoS](#iter-24)
...
<a id="iter-1"></a>
### Таблица маршрутов (distance vector с withdrawal)

**Статус: базовая реализация завершена.** Distance-vector routing с per-origin SeqNo, withdrawal'ами, triggered update'ами, split horizon, flap detection, hop_ack confirmation и RPC observability полностью реализованы и протестированы. Полная документация: [`routing.md`](routing.md), [`rpc/routing.md`](rpc/routing.md), [`protocol/relay.md`](protocol/relay.md).

Оставшиеся задачи в рамках этой итерации:
...
**Архитектура DMRouter:**

- [ ] Выделить интерфейс клиента для DMRouter (заменить конкретный `*DesktopClient` на interface — тестируемость, mockability)
- [ ] DMRouter остаётся чистым роутером сообщений + UI (file transfer orchestration уже вынесена в `FileTransferBridge`; убрать оставшиеся неявные зависимости)

**Протокол передачи файлов:**

- [ ] Компенсирующая команда `file_announce_cancel`: если Commit на стороне отправителя упал после доставки DM, отправить cancel получателю чтобы он удалил карточку файла вместо ожидания timeout'а chunk_request
- [ ] Реализовать подсчёт банов для direct-connect пиров: невалидные файловые команды → ban score → disconnect на threshold
- [ ] Реализовать состояние `TemporarilyUnavailable` с grace period (`FileUnavailableGracePeriod`, 24ч по умолчанию)
- [ ] Реализовать локальное состояние `Evicted` получателя (только локально, без протокольного сигнала)
- [ ] Реализовать лимиты ресурсов: `MaxPartialDownloadStorage` с LRU-вытеснением
- [ ] Реализовать периодический GC transmit-директории (часовое сканирование, удаление orphan'ов старше 7 дней) — сейчас очистка выполняется только при запуске
- [ ] Unit-тест: ban score НЕ увеличивается для релеиз-невалидной файловой команды (directPeer ≠ SRC)
- [ ] Unit-тест: пир отключается когда ban score достигает threshold
- [ ] Unit-тест: state machine отправителя — файл удалён → Unavailable → tombstone на запрос неизвестного FileID
- [ ] Unit-тест: LRU-вытеснение MaxPartialDownloadStorage
- [ ] Unit-тест: валидация при запуске — недоступная директория → TemporarilyUnavailable (не tombstone)
- [ ] Unit-тест: TemporarilyUnavailable → active когда файл появляется
- [ ] Unit-тест: TemporarilyUnavailable → tombstone после grace period
- [ ] Unit-тест: LRU-вытеснение → Evicted состояние (локально, никакого протокольного сигнала)
- [ ] Unit-тест: Evicted → Available при пользовательском переприёме (перезагрузка с 0)
- [ ] Интеграционный тест: полный жизненный цикл (announce DM → download via file commands → verify → file_downloaded + file_downloaded_ack)
- [ ] Интеграционный тест: возобновление после прерывания с последнего offset
- [ ] Интеграционный тест: duplicate file_downloaded → sender re-sends file_downloaded_ack (idempotent)
- [ ] Интеграционный тест: file_downloaded_ack timeout → receiver retries with backoff until ACK or completes locally
- [ ] Интеграционный тест: отправитель отменяет mid-transfer (удаление file_announce DM → FileMapping tombstoned → transmit-файл освобождён если RefCount=0)
- [ ] Интеграционный тест: получатель отменяет mid-transfer (удаление file_announce DM → partial download очищен → FileMapping удалён)
- [ ] Интеграционный тест: файловые команды передачи файлов не блокируют DM-доставку под нагрузкой
- [ ] Интеграционный тест: нет маршрута → WaitingRoute, файловые команды молча отбрасываются (без gossip)
- [ ] Интеграционный тест: capability gate — команды передачи файлов никогда не отправляются legacy-пирам
- [ ] Тест смешанных версий: узел с файлами рядом с legacy-узлом — никакой утечки трафика файлов к legacy
...
**Механизм — delete-message DM для `file_announce`:**

При удалении `file_announce` сообщения (с любой стороны):

1. `FileMapping`, связанный с FileID этого сообщения, удаляется
   (переходит в tombstone).
2. Если удалённый mapping был последней ссылкой на transmit-файл
   (`RefCount` падает до 0), transmit-файл удаляется с диска.
3. Если другие `FileMapping` всё ещё ссылаются на тот же transmit-файл
   (один контент отправлен нескольким получателям), файл сохраняется —
   удаляется только mapping.
4. Другая сторона получает delete-message DM и выполняет аналогичную
   очистку: отправитель удаляет FileMapping + transmit-файл (если
   RefCount=0); получатель удаляет частичную загрузку и FileMapping.

Нет сообщения — нет файла.

**Зависимость:** требует передачу файлов (см. [protocol/file_transfer.md](protocol/file_transfer.md)) для
структур `FileMapping`, `RefCount` и хранения transmit-файлов.
...
<a id="iter-10"></a>
### Резервные каналы через Google WSS
...
<a id="iter-2"></a>
### Надёжность, репутация, multi-path и инкрементальная синхронизация
...
<a id="iter-4"></a>
### Структурированный overlay (DHT) (будущее)
...
- [ ] Унификация типа `MessageID`: перенести `protocol.MessageID` в пакет `domain`, чтобы все слои использовали единый типизированный ID без циклических импортов. Обновить `protocol.Envelope`, `protocol.DeliveryReceipt`, записи chatlog и сериализацию фреймов на использование `domain.MessageID`. Сейчас `domain.MessageID` и `protocol.MessageID` — независимые `string`-алиасы, которые должны быть объединены.
...
<a id="iter-8"></a>
### Второй слой шифрования и обмен ключами
...
<a id="iter-13-3"></a>
#### Эфемерные ключи и прямая секретность
...
<a id="iter-14"></a>
### Глобальные имена вместо raw identity
...
<a id="iter-17-1"></a>
#### BLE-транспорт и обнаружение пиров
...
<a id="iter-17-2"></a>
#### BLE-фрагментация и адаптация к MTU
...
<a id="iter-18-2"></a>
#### Radio-aware фрагментация и pacing
...
<a id="iter-18-3"></a>
#### Radio mesh relay и дедупликация
...
<a id="iter-19"></a>
### Конструктор кастомного шифрования
...
#### Классификация адресов и приоритет маршрутизации
...
#### Rate limiting и защита от усиления
...
#### Протокол координации hole punch
...
#### Предсказание портов symmetric NAT
...
#### Апгрейд соединения с relay на прямое
...
**Зависимость:** улучшает передачу файлов и голосовые вызовы
...
#### Per-connection состояние перегрузки
...
#### Алгоритм адаптации скорости AIMD
...
#### Классы приоритета и классификация пакетов
...
| **Bulk** (обычный) | Сообщения, данные файлов, анонсы маршрутов, синхронизация | Подчиняются congestion control. Ставятся в очередь и rate-limited. Уступают bandwidth высшим классам. | `dm_message`, файловые команды `chunk_response`, `route_announce`, `group_sync` |
...
#### Архитектура приоритетной очереди
...
#### Обнаружение перегрузки и оценка RTT
...
sequenceDiagram
    ...
    A->>B: DM(Command=chunk_response) (t=5мс)
    ...
    A->>A: Ограничить файловые команды chunk_response до<br/>BulkBudget() за тик
...
#### Backpressure и интеграция с relay
...
#### Взаимодействие congestion control и передачи файлов

Передача файлов — основной потребитель bulk-bandwidth.
Контроллер перегрузки гарантирует, что данные файлов не голодают остальной трафик:

```mermaid
flowchart TD
    FT[Движок передачи файлов<br/>генерирует DM chunk_response] --> BQ[Очередь bulk]
    ...
    WAIT --> PAUSE[Авто-пауза загрузок<br/>если очередь полна > 5с]
...
| Resume передачи файлов через FileID (DM MessageID), целостность через FileHash (SHA-256) | FileID (UUID DM-анонса) идентифицирует событие передачи для resume. FileHash (SHA-256 контента файла) верифицирует целостность после сборки. Намеренно разделены: один файл отправленный дважды = два FileID, один FileHash. |
