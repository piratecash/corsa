...
- [Итерация A2 — Удаление сообщений](#iter-a2)
- [Итерация A3 — Android-приложение](#iter-a3)
- [Итерация A4 — Второй слой шифрования](#iter-a4)
- [Итерация A5 — Обход DPI](#iter-a5)
- [Итерация A6 — Google WSS fallback](#iter-a6)
- [Итерация A7 — SOCKS5-туннель](#iter-a7)
- [Итерация A8 — Групповые чаты](#iter-a8)
- [Итерация A9 — Onion DM](#iter-a9)
  - [A9.1 — Onion-обёртка DM envelope](#iter-a9-1)
  - [A9.2 — Onion-квитанции](#iter-a9-2)
  - [A9.3 — Эфемерные ключи и прямая секретность](#iter-a9-3)
  - [A9.4 — Выбор пути и ротация цепочек](#iter-a9-4)
  - [A9.5 — Защита от анализа трафика](#iter-a9-5)
- [Итерация A10 — Глобальные имена](#iter-a10)
- [Итерация A11 — Расширения Gazeta](#iter-a11)
- [Итерация A12 — iOS-приложение](#iter-a12)
- [Итерация A13 — BLE last mile](#iter-a13)
  - [A13.1 — BLE-транспорт и обнаружение пиров](#iter-a13-1)
  - [A13.2 — BLE-фрагментация и адаптация к MTU](#iter-a13-2)
  - [A13.3 — BLE-дедупликация и relay](#iter-a13-3)
  - [A13.4 — BLE rate limiting и QoS](#iter-a13-4)
- [Итерация A14 — Meshtastic last mile](#iter-a14)
  - [A14.1 — Meshtastic transport bridge](#iter-a14-1)
  - [A14.2 — Radio-aware фрагментация и pacing](#iter-a14-2)
  - [A14.3 — Radio mesh relay и дедупликация](#iter-a14-3)
- [Итерация A15 — Конструктор шифрования](#iter-a15)
- [Итерация A16 — Голосовые звонки](#iter-a16)
...
<a id="iter-1"></a>
### Итерация 1 — Hop-by-hop relay (с ограничением по capabilities)
...
**Ограничение по capabilities:** `relay_message` отправляется только peer'ам,
...
- [x] Написать unit-тесты для ограничения по capabilities (legacy peer получает gossip, не relay)
...
- [ ] Ограничить размер входящего relay-фрейма и число in-flight `relayForwardState` на peer / на ноду
- [ ] Добавить bounded queue/backpressure для relay retry, чтобы flood не раздувал `queue-{port}.json` бесконечно
- [ ] Добавить per-peer token bucket для gossip/relay fan-out под нагрузкой
- [ ] Добавить overload-тест: flood `relay_message` не приводит к unbounded росту памяти/диска
...
- [ ] Централизовать admission control: frame size limits, handshake timeout, session quotas, overload-mode hooks
...
подписями. Но Iteration 2 должна оставаться работоспособной и без
...
- [ ] Добавить anti-poisoning acceptance rules: `announcement` advisory-only, без переопределения `direct`/`hop_ack` устаревшими данными
- [ ] Отклонять или депривилегировать аномальные route-анонсы (неправдоподобный `hops`, резкий всплеск новых identity, отсутствие свежего `SeqNo`)
- [ ] Ограничить rate `announce_routes` / `withdrawal` на peer, чтобы triggered updates не превращались в routing-flood
- [ ] Ввести квоты на число новых identity и route entries, которые один peer может привнести за окно времени
- [ ] Добавить jitter / pacing для periodic full sync, чтобы ноды не синхронизировали всплески одновременно
...
- [ ] Интеграционный тест: malicious peer рекламирует ложные маршруты, доставка деградирует не более чем до gossip fallback
- [ ] Интеграционный тест: route-update flood не выбивает честные peer'ы и не делает full-sync storm
...
```mermaid
graph LR
    I1["Итерация 1<br/>Hop-by-hop relay<br/>(с ограничением по capabilities)"]
...
<a id="iter-a2"></a>
### Итерация A2 — Управление удалением сообщений
...
<a id="iter-a3"></a>
### Итерация A3 — Android-приложение
...
<a id="iter-a4"></a>
### Итерация A4 — Второй слой шифрования и обмен ключами
...
<a id="iter-a6"></a>
### Итерация A6 — Резервные каналы через Google WSS
...
<a id="iter-a7"></a>
### Итерация A7 — SOCKS5-туннель между identity
...
- **Изменения состава подписаны.** Добавление или удаление участника требует
  ...
  группы.
- **Per-group симметричный ключ.** Сообщения внутри группы шифруются общим
  групповым ключом, выведенным через раунд key-agreement среди текущих
  участников. При изменении состава групповой ключ ротируется (forward secrecy
  для удалённых участников).
- **Модель доставки:** отправитель шифрует один раз групповым ключом, затем
  рассылает через mesh relay (один `relay_message` на участника или gossip в
  group topic). Семантика квитанций: per-member delivery receipts, в UI
  агрегируются как «доставлено N из M».
...
консистентным составом, ротацией ключа при изменении состава и delivery
receipts — всё работает поверх существующего mesh relay без центрального
сервера.

<a id="iter-a9"></a>
### Итерация A9 — Onion-доставка DM
...
<a id="iter-a9-1"></a>
#### A9.1 — Onion-обёртка DM envelope

**Цель:** ни один промежуточный хоп не видит адрес получателя или содержимое
DM.

**Как это работает:** отправитель знает путь из routing table (Итерация 2) и
...
2. **Circuit rebuild (sender-level, длинный):** отправитель ожидает
   ...
   Итерация 3). Если ack не приходит, отправитель помечает circuit как
...
routing announcements (Итерация 2) — это основной и единственный механизм
в A9.1. Отдельный фрейм `onion_key_announce` **не** вводится: он
усложнит capability-модель, увеличит размер announce-кадров и создаст
...
1. **Источник ключа:** transit box key приходит из routing announcement
   (Итерация 2). Announcement подписан ed25519-ключом узла — identity
   self-authenticating.
...
3. **Версионирование и anti-replay:** binding payload включает формат
   ...
   (Итерация 2): каждый новый announcement имеет seq > предыдущего.
...
<a id="iter-a9-3"></a>
#### A9.3 — Эфемерные ключи и прямая секретность
...
<a id="iter-a9-4"></a>
#### A9.4 — Выбор пути и ротация цепочек
...
нода лично наблюдала успешные hop-ack'и (Итерация 3, `ReliabilityScore`).
...
<a id="iter-a9-5"></a>
#### A9.5 — Защита от анализа трафика
...
Итерация 4e определяет глобальный overload mode: снижение gossip fan-out,
...
| 2 | Routing announcements / hop-ack | Частота снижается (Итерация 4e), но не блокируется |
...
| 6 (низший) | Non-critical sync / delta announces | Откладывается (Итерация 4e) |
...
3. Глобальный overload (Итерация 4e активна): onion уже в режиме
...
#### Интеграционные тесты для A9 (cross-sub-iteration)
...
#### Hostile-Internet Guardrails в применении к A9
...
<a id="iter-a10"></a>
### Итерация A10 — Глобальные имена вместо raw identity
...
<a id="iter-a11"></a>
### Итерация A11 — Расширения протокола Gazeta
...
(Итерация 2) для multi-hop BLE-путей.
...
<a id="iter-a13-1"></a>
#### A13.1 — BLE-транспорт и обнаружение пиров
...
<a id="iter-a13-2"></a>
#### A13.2 — BLE-фрагментация и адаптация к MTU
...
<a id="iter-a13-3"></a>
#### A13.3 — BLE-дедупликация и relay
...
<a id="iter-a14"></a>
### Итерация A14 — Meshtastic last mile
...
<a id="iter-a14-2"></a>
#### A14.2 — Radio-aware фрагментация и pacing
...
<a id="iter-a14-3"></a>
#### A14.3 — Radio mesh relay и дедупликация
...
<a id="iter-a15"></a>
### Итерация A15 — Конструктор кастомного шифрования
...
graph LR
    ...
    A15 --> A16["A16<br/>Голосовые звонки"]
