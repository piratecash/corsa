...
Исходники (планируемые): `internal/core/node/routing_table.go`, `internal/core/node/route_announce.go`
...
- [Итерация 2 — Надёжность, репутация и multi-path](#iter-2)
...
- [ ] Подтверждено: итерация 1 остаётся аддитивной, protocol bump не требуется
- [ ] Подтверждено: итерация 1 не требует повышения `MinimumProtocolVersion`

<a id="iter-2"></a>
### Итерация 2 — Надёжность, репутация и multi-path
