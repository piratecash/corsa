...
- [Итерация 1 — Таблица маршрутов](#iter-1)
  - [1.1 — Инварианты модели](#iter-1-1)
  - [1.2 — Минимальный вертикальный slice](#iter-1-2)
  - [1.3 — Наблюдаемость и polish](#iter-1-3)
  - [1.4 — Health, probes, RTT, route query](#iter-1-4)
- [Итерация 1.5 — Здоровье маршрутов, probe'ы и RTT-скоринг](#iter-1-5)
...
Hop-by-hop relay реализован и стабилен (capability `mesh_relay_v1`).
Сообщение от ноды A теперь может пройти через промежуточные ноды
(A→B→C→D→E→F) до получателя, которого не знает ни один прямой peer A.
Лимиты ёмкости, per-peer rate limiting, дедупликация и delivery receipts
через обратный relay-путь — всё на месте. Детали протокола:
[`relay.md`](protocol/relay.md).

```mermaid
graph LR
    A["Нода A<br/>(отправитель)"] -->|relay| B["Нода B"]
    B -->|relay| C["Нода C"]
    C -->|relay| F["Нода F<br/>(получатель)"]
    F -.->|receipt| C -.->|receipt| B -.->|receipt| A

    style A fill:#e3f2fd,stroke:#1565c0
    style F fill:#c8e6c9,stroke:#2e7d32
```
*Диаграмма — Hop-by-hop relay с обратным путём для receipt (реализовано)*

**Что ещё не сделано:** relay работает, но нода не знает, **куда**
пересылать. Forwarding слепой — gossip fan-out идёт к top-scored peer'ам
без знания, какое направление ведёт к получателю. Следующий шаг
(Итерация 1) добавляет таблицу маршрутов, чтобы каждая нода знала,
какие identity доступны через каких соседей, превращая слепой gossip
в направленную пересылку.
...
internal/core/routing/
  types.go            — RouteEntry (с полем Origin), RoutingSnapshot (экспортируемые типы)
...
Данные маршрутизации (состояние таблицы, здоровье, статистика)
предоставляются через существующую `CommandTable` посредством
интерфейса `RoutingProvider`:

```go
// RoutingProvider абстрагирует доступ к подсистеме маршрутизации.
// Доступен только при активном mesh_routing_v1 capability.
type RoutingProvider interface {
    // FetchRoutes возвращает маршруты для конкретного identity или все
    // маршруты, если identity пуст. Возвращает snapshot — безопасно для сериализации.
    FetchRoutes(identity string) []RoutingEntry

    // FetchRoutingStats возвращает агрегированную статистику:
    // размер таблицы, метрики сходимости, счётчики announce/withdrawal.
    FetchRoutingStats() RoutingStatsEntry

    // FetchRouteHealth возвращает состояние здоровья per (identity, origin, nextHop) triple.
    // Добавлено в итерации 1.5; возвращает пустой результат до активации трекинга здоровья.
...
| Команда | Описание | Использование | Начиная с |
|---|---|---|---|
| `fetch_routes` | Маршруты для конкретного identity или вся таблица | `[identity]` | Итер 1 |
| `fetch_routing_stats` | Размер таблицы, счётчики анонсов, время сходимости | — | Итер 1 |
| `fetch_route_health` | Состояния здоровья per (identity, origin, nextHop) triple | — | Итер 1.5 |

Регистрация следует существующему паттерну:
...
Когда `mesh_routing_v1` не активен (legacy-нода), все routing-команды
возвращают 503 (unavailable) — аналогично существующим mode-gated
командам вроде `fetch_traffic_history`.

`RegisterAllCommands` получает параметр `RoutingProvider`:
...
Реализация разбита на четыре последовательные фазы. Каждая фаза должна
быть полностью завершена (код + тесты проходят) перед началом следующей.

<a id="iter-1-1"></a>
#### Фаза 1.1 — Инварианты модели

**Что меняется:** сейчас в кодовой базе нет модели данных маршрутизации —
сообщения доставляются через gossip (flood на всех пиров). Эта фаза
вводит фундаментальные типы и правила, на которые опираются все
последующие фазы: `RouteEntry` с полем `Origin`, ключ дедупликации
`(identity, origin, nextHop)`, split horizon, семантика withdrawal,
capability gates и TTL expiry.

**Почему первая:** все последующие фазы (таблица, анонсы, интеграция)
обязаны соблюдать эти инварианты. Ошибка здесь означает каскадные баги
в фазах 1.2–1.4. Изолируя инварианты с полным тестовым покрытием до
любого сетевого кода, мы гарантируем твёрдый фундамент.

**Что было → что стало:**

| До (только gossip) | После (Фаза 1.1) |
|---|---|
| Нет типа `RouteEntry` | `RouteEntry` с Identity, Origin, NextHop (Ed25519), Hops, SeqNo, RemainingTTL, Source |
| Нет дедупликации — relay dedupe только по `message_id` | Origin-aware дедупликация: ключ `(identity, origin, nextHop)` |
| Нет концепции withdrawal | `hops=16` = withdrawn; wire withdrawal только origin; transit = локальная инвалидация |
| Нет split horizon | `Announceable(excludeVia)` пропускает маршруты, полученные от целевого пира |
| Нет capability gate для фреймов маршрутизации | `announce_routes` gated по `mesh_routing_v1` |
...
```
*Диаграмма — Базовые типы фазы 1.1 и их связь*
...
<a id="iter-1-2"></a>
#### Фаза 1.2 — Минимальный вертикальный slice

**Что меняется:** подключить модель данных из фазы 1.1 к реальной сети.
После этой фазы сообщения между несмежными пирами будут ретранслироваться
через таблицу маршрутов вместо gossip flood. Впервые в логах появится
`route_via_table`.

**Что покрывает:** структура `Table` с `Lookup()`, периодические анонсы
(30с), triggered updates при connect/disconnect, обработка входящих
анонсов, адаптер `TableRouter` подключённый к существующему
`Router.Route()`, инвалидация при закрытии сессии, подтверждение hop_ack
и mixed-version сосуществование с gossip-only пирами.

**Что было → что стало:**

| До (только gossip) | После (Фаза 1.2) |
|---|---|
| `Router.Route()` возвращает только gossip targets | `Router.Route()` заполняет `RelayNextHop` из таблицы, fallback на gossip |
| Нет `announce_routes` на проводе | Периодические + triggered `announce_routes` пирам с `mesh_routing_v1` |
| Disconnect = просто удалить сессию | Disconnect = wire withdrawal (own-origin) + локальная инвалидация (transit) + triggered update |
| Нет подтверждения маршрутов | `relay_hop_ack` продвигает маршрут до `source="hop_ack"` для конкретного triple |
...
<a id="iter-1-3"></a>
#### Фаза 1.3 — Наблюдаемость и polish

**Что меняется:** делаем таблицу маршрутов наблюдаемой и устойчивой.
После этой фазы десктопный UI, CLI и мониторинг смогут запрашивать
маршруты и статистику через RPC. Fairness rotation обеспечивает
распространение далёких identity. Anti-poisoning правила защищают от
инъекции ложных маршрутов, которые могут переопределить проверенные.

**Почему отдельно от 1.2:** Фаза 1.2 даёт минимально работающую
маршрутизацию. Добавление RPC, fairness и anti-poisoning в ту же фазу
создаёт связность корректности с polish. Разделение позволяет
отправить и провалидировать Фазу 1.2 до добавления сложности.

**Что было → что стало:**

| До (только Фаза 1.2) | После (Фаза 1.3) |
|---|---|
| Нет способа проверить маршруты в рантайме | `fetch_routes`, `fetch_routing_stats` через RPC |
| Announce несёт полную таблицу (до 100) по hop count | Fairness rotation: direct всегда включены, offset rotation для остальных |
| Любой `announcement` может переопределить `hop_ack` | Trust hierarchy: `direct > hop_ack > announcement` |
| Нет защиты от route flooding | Rate limits, квоты, jitter на periodic full sync |

```mermaid
flowchart TD
    subgraph RPC["RPC Routing Commands"]
        FR["fetch_routes<br/>(фильтр по identity)"]
        FS["fetch_routing_stats<br/>(размер таблицы, счётчики)"]
        FH["fetch_route_health<br/>(Итер. 1.5)"]
    end

    subgraph Provider["Интерфейс RoutingProvider"]
...
<a id="iter-1-4"></a>
#### Фаза 1.4 — Итерация 1.5 (health, probes, RTT, route query)

**Что меняется:** замена простого ранжирования по hop count на композитный
score, учитывающий здоровье маршрута, RTT и доверие. Добавляется активный
health tracking по `(identity, origin, nextHop)` тройке с 4-state machine
(Good → Questionable → Bad → Dead), лёгкие probe'ы для верификации
reachability, и целевые route query для быстрого восстановления.

**Почему отдельная фаза:** health tracking и composite scoring добавляют
существенную сложность. Если смешать с Фазами 1.1–1.3, баг в health-логике
может заблокировать доставку базовой таблицы маршрутов. Вынося в Фазу 1.4,
мы гарантируем, что базовая маршрутизация уже работает и провалидирована.

**Что было → что стало:**

| До (Фаза 1.3) | После (Фаза 1.4) |
|---|---|
| Выбор маршрута: hop count + trust source | Композитный score: hops + RTT + health + trust |
| Нет health state на маршрут | 4-state machine: Good/Questionable/Bad/Dead по тройке |
| Нет механизма probe | `route_probe` / `route_probe_ack` (gated by `mesh_route_probe_v1`) |
| Нет активного обнаружения маршрутов после сбоя | `route_query` / `route_query_response` (gated by `mesh_route_query_v1`) |
| RTT неизвестен | RTT из hop_ack timing и tcp_info (EWMA) |

Начинать только после полного завершения фаз 1.1–1.3 и прохождения всех тестов.
Health tracking и composite scoring не должны блокировать базовую корректную
таблицу маршрутов.

<a id="iter-1-5"></a>
### Итерация 1.5 — Здоровье маршрутов, probe'ы и RTT-скоринг
...
**Зависит от:** Итерации 1 фазы 1.1–1.3 (таблица маршрутов с announce'ами
и withdrawal'ами должна работать, все тесты проходят).
