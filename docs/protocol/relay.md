...
**INV-1/INV-2 exception — forward-once relays (opt-in, `CORSA_TRANSIT_FORWARD_ONCE`).** When enabled, a relay does NOT buffer transit DMs at all: `storeIncomingMessage` forwards the frame in a single pass (the usual gossip + relay sends — deduped at the receiver; forward-once bounds buffering/re-gossip over TIME, not the per-pass frame count) and neither appends it to `topics["dm"]` nor registers a `relayRetry` entry — so `retryRelayDeliveries` has nothing to re-emit and the in-flight buffer disappears. This removes the transit gossip-storm at its source; durability falls entirely on the SENDER's receipt-driven retry (`awaitingDelivered`, cleared only by a `delivered`/`seen` receipt — NOT by `relay_hop_ack` or `ack_delete` — and not bounded by any attempt count). Semantics note: in this mode `storeIncomingMessage` still returns `stored=true` and the node still emits `ack_delete` / a `"stored"` hop-ack, which here mean "ACCEPTED / already seen (dedup), stop pushing me this id" — NOT "buffered for retry". That is safe because the previous hop's `ack_delete`/hop-ack only releases its own relay backlog, while the origin's `awaitingDelivered` retry is receipt-driven and independent, so it keeps redelivering until the recipient confirms (ended only by that confirmation, by the author's withdrawal, or by the message's own TTL — the online-overlap model). A dedicated "forwarded-not-buffered" wire status would make this explicit but requires protocol-version negotiation (future work). Default OFF; the legacy in-flight buffer (INV-1/INV-2) applies until enabled.

The relay-retry contour (`trackRelayMessage` / `retryableRelayMessages` / `retryRelayDeliveries`) operates on **transit** envelopes only (neither sender nor recipient is this node). An **origin-authored** DM (sender is this node) is never tracked or re-gossiped through this contour — its re-send is owned exclusively by the sender-owned delivery engine (`registerAwaitingDelivered` → `retryDueDeliveries` → `dispatchEnvelopeRetry`), which paces itself on a capped backoff and ends only on confirmation, withdrawal or the message's own TTL. Tracking origin DMs in the relay-retry contour previously re-gossiped them indefinitely: each mesh echo re-armed the `relayRetryTTL` window, and the re-emission was BLIND — it flooded the mesh whether or not the recipient was reachable, which surfaced as multi-day re-circulation of a node's own undelivered DMs. The sender-owned engine also has no attempt ceiling, and that is not the same defect: it emits only into a route that exists, so a message waiting for an absent recipient produces no wire traffic at all. The same rule is enforced on the FAST relay path: `handleRelayMessage` drops a received `relay_message` whose origin sender is this node (an echo of our own DM, recipient ≠ self) before any forward — the fast path forwards before `storeIncomingMessage`, so it cannot rely on the store-path `originEcho` guard.
...
**INV-3 exception — reachability-gated origin sends (default ON, `CORSA_HOLD_DM_UNTIL_REACHABLE`).** For messages THIS node authored (sender == self) the invariant is narrowed: the origin first send and every sender-owned retry emit only when the recipient is REACHABLE — a directed route (`RelayNextHop`) exists or the recipient is a directly connected subscriber. An unreachable recipient is HELD in `awaitingDelivered` (no blind gossip into the void) and delivered when a route or connection appears — `kickDeliveryRetriesForReachable`, fired from the announce/connect drain, re-arms held entries immediately. TRANSIT forwarding (sender != self) is NOT affected — blind gossip remains the unconditional propagation mechanism for other people's messages. The hold has no age limit of its own: waiting costs nothing on the wire, because a held message emits nothing at all. This is the cure for the blind-gossip storm to offline/long-gone recipients (months-old undelivered DMs re-flooding the mesh). Enabled by default; `CORSA_HOLD_DM_UNTIL_REACHABLE` set to a falsey value is a kill-switch that restores the legacy unconditional blind-gossip baseline. Trade-off: a recipient that is online but whose route has not yet propagated to this node is delivered only once its announce arrives (no blind discovery), instead of immediately via flood.

**INV-3 exception 2 — transit age ceiling REMOVED in protocol v30.** A relay no longer refuses a transit DM for being old, there is no setting that brings the refusal back (`CORSA_TRANSIT_MAX_AGE_HOURS` and `config.TransitMaxAge` are gone), and this is a reversal. The ceiling could not distinguish an envelope circulating on its own from a live sender re-sending an old message to a recipient who is online — it refused both, and the refusal is silent by contract (no hop-ack), so the sender read it as a dead uplink, charged the route a failure via `MarkHopFailure`, failed over to another uplink and was refused there too. A DM older than a day could never cross a relay again, no matter how many times its author tried, while working routes were being degraded by every attempt. What bounds circulation instead is everything that measures the FORWARDING rather than the message: the hop budget, `max_hops`, forward-once, the transit in-flight window and its per-recipient rings, ingress suppression and the origin-echo drop. Keeping an operator knob would have been worse than useless: such a node advertises v30 — "I carry old messages" — while behaving like a pre-v30 one, and once the floor rises and the sender-side re-stamp is deleted it becomes a silent black hole no sender can diagnose.

Two rules survive the change, both about the frame rather than its age. A transit DM carrying NO `created_at` is refused: the relay path skips timestamp validation and the outer field is unsigned, so a frame without one cannot be reasoned about at all. A transit DM dated further into the future than clock skew allows is refused as a forgery — that test used to live inside the age check and is now applied on its own (`envelopeFutureDated`).

Until `MinimumProtocolVersion` reaches 30, a transit message carries a fresh `created_at`, normalised at the DOORS — `storeIncomingMessage` (admission), `handleRelayMessage` (the fast path, which forwards before admission) and `dispatchEnvelopeRetry` (this node's own outbox) — after which every frame built simply copies it (`legacyTransitStamp`, `TODO(transit-age-restamp-removal)`). The TTL is re-derived with the date, so the author's absolute deadline is preserved rather than extended at every hop, and a message already past its deadline is not re-dated at all. Forwarding is included deliberately: in a v29 → v30 → v29 route the middle node accepts the old message and would otherwise pass it on unchanged for the next node to drop. BROADCAST is excluded — it keeps a real ceiling of its own, and re-dating it on every hop would hand it a fresh lifetime, which is the re-circulation bypass this layer exists to prevent. The field is not covered by the DM signature, the id is unchanged, and this node's own record — the chatlog row, the outbox entry — keeps the real date. What is never re-dated is this node's own RECORD — the chatlog row it stores for a message addressed to it, and the outbox entry for one it authored. The wire copy of an authored message IS re-dated when it goes out (`dispatchEnvelopeRetry`), for the same reason a forwarded one is. What remains of the retention layer for the DM classes is the two frame-shape refusals above; the `StoredAt` in-flight window still bounds the SHORT forwarding operation, and the hop budget, `max_hops`, forward-once and the per-recipient transit rings bound circulation. BROADCAST keeps a real ceiling, `BroadcastMaxAge` (default 24h, `CORSA_BROADCAST_MAX_AGE_HOURS`), anchored on the IMMUTABLE sender `CreatedAt` rather than the local `StoredAt`: the legacy `transitInFlightWindow` was `StoredAt`-anchored, which `storeIncomingMessage` re-stamps on every admission, so after the rotating bloom forgot an id (5–10 min) a re-injected envelope reset its window and circulated forever — the original storm. Classification and policy live in `envelope_retention.go`; `CORSA_ENVELOPE_RETENTION` falsey disables the broadcast ceiling too.
...
**INV-3 исключение — отправка origin по достижимости (по умолчанию ВКЛ, `CORSA_HOLD_DM_UNTIL_REACHABLE`).** Для сообщений, автором которых является ЭТОТ узел (sender == self), инвариант сужается: первая отправка origin и каждый sender-owned ретрай эмитят, только когда получатель ДОСТИЖИМ — есть directed-маршрут (`RelayNextHop`) или прямой подписчик-коннект. Недостижимый получатель УДЕРЖИВАЕТСЯ в `awaitingDelivered` (без blind-gossip в пустоту) и доставляется при появлении маршрута/коннекта — `kickDeliveryRetriesForReachable` (из announce/connect drain) немедленно пере-армливает удержанные записи. ТРАНЗИТНАЯ пересылка (sender != self) НЕ затрагивается — blind-gossip остаётся безусловным механизмом распространения чужих сообщений. У удержания нет собственной возрастной границы: ожидание ничего не стоит на проводе, потому что удержанное сообщение не эмитит вообще ничего. Это лекарство от blind-gossip-шторма к офлайн/исчезнувшим получателям (месячные недоставленные DM, заново заливающие mesh). Включено по умолчанию; `CORSA_HOLD_DM_UNTIL_REACHABLE` в falsey — kill-switch, возвращающий легаси безусловный blind-gossip. Компромисс: получатель, который онлайн, но чей маршрут до нас ещё не дошёл, доставляется только по приходу его анонса (без blind-discovery), а не мгновенно через flood.

**INV-3 исключение 2 — возрастной потолок транзита УДАЛЁН в протоколе v30.** Релей больше не отказывается нести транзитный DM из-за возраста, вернуть отказ настройкой нельзя (`CORSA_TRANSIT_MAX_AGE_HOURS` и `config.TransitMaxAge` удалены), и это отмена прежнего решения. Потолок не мог отличить конверт, кружащий по сети сам по себе, от живого отправителя, переотправляющего старое сообщение получателю, который сейчас в сети, — он отбрасывал оба, причём молча (по контракту без hop-ack), поэтому отправитель читал отказ как мёртвый аплинк, записывал маршруту отказ через `MarkHopFailure`, уходил на другой аплинк и получал там то же самое. DM старше суток не мог пройти через релей больше никогда, сколько бы раз автор ни пытался, а исправные маршруты деградировали с каждой попыткой. Циркуляцию ограничивает всё то, что измеряет ПЕРЕСЫЛКУ, а не сообщение: hop-бюджет, `max_hops`, forward-once, транзитное in-flight окно и кольца на получателя, подавление ingress и дроп origin-echo. Оставить операторский переключатель было бы хуже, чем бесполезно: такой узел рекламирует v30 — «я несу старые сообщения» — а ведёт себя как узел до v30, и как только минимальная версия сети поднимется и ре-штамп на стороне отправителя будет удалён, он превратится в молчаливую чёрную дыру, которую отправитель никак не диагностирует.

Изменение переживают два правила, и оба о кадре, а не о его возрасте. Транзитный DM БЕЗ `created_at` отвергается: на релейном пути валидация времени пропускается, а внешнее поле не подписано, поэтому о таком кадре вообще нельзя ничего утверждать. Транзитный DM с датой дальше в будущем, чем допускает часовой дрейф, отвергается как подделка — эта проверка раньше жила внутри возрастной и теперь применяется отдельно (`envelopeFutureDated`).

Пока `MinimumProtocolVersion` не достигнет 30, транзитное сообщение несёт свежий `created_at`, нормализованный НА ДВЕРЯХ — `storeIncomingMessage` (admission), `handleRelayMessage` (быстрый путь, форвардящий до admission) и `dispatchEnvelopeRetry` (собственный исходящий), — а каждый построенный дальше кадр её просто копирует (`legacyTransitStamp`, `TODO(transit-age-restamp-removal)`). TTL пересчитывается вместе с датой, поэтому абсолютный дедлайн автора сохраняется, а не продлевается на каждом хопе; сообщение, уже перешагнувшее дедлайн, не переставляется вовсе. Форвард включён намеренно: на маршруте v29 → v30 → v29 промежуточный узел принимает старое сообщение и иначе передал бы его без изменений, чтобы следующий его выбросил. BROADCAST исключён — у него сохраняется собственный потолок, и переустановка даты на каждом хопе выдавала бы ему новую жизнь, то есть ровно тот обход защиты от повторной циркуляции, ради которого слой и существует. Поле не покрыто подписью DM, идентификатор не меняется, а собственная запись узла — строка chatlog, запись исходящего — сохраняет настоящую дату. Никогда не переставляется собственная ЗАПИСЬ узла — строка chatlog для сообщения, адресованного ему, и запись исходящего для им авторствованного. Уходящая в сеть копия авторствованного сообщения переставляется (`dispatchEnvelopeRetry`) ровно по той же причине, что и пересылаемого. От слоя ретенции для DM-классов остаются те же два отказа по форме кадра; `StoredAt`-окно по-прежнему ограничивает КОРОТКУЮ операцию пересылки, а циркуляцию — hop-бюджет, `max_hops`, forward-once и транзитные кольца на получателя. У BROADCAST остаётся настоящий потолок `BroadcastMaxAge` (по умолчанию 24ч, `CORSA_BROADCAST_MAX_AGE_HOURS`), якорящийся на НЕИЗМЕНЯЕМОМ `CreatedAt` отправителя, а не на локальном `StoredAt`: легаси `transitInFlightWindow` якорился как раз на `StoredAt`, который `storeIncomingMessage` переставляет на каждом admission, поэтому после того как ротационный bloom забывал id (5–10 мин), ре-инжектнутый конверт сбрасывал окно и кружил вечно — исходный шторм. Классификация и политика — в `envelope_retention.go`; `CORSA_ENVELOPE_RETENTION` в falsey выключает и broadcast-потолок.
