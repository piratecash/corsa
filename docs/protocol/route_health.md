...
- **Periodic idle tick**: every `HealthProbeInterval` (30 s) the
...
- Probes are scheduled by the sender every `HealthProbeInterval`
  (30 s) for `(Identity, Uplink)` pairs currently in `Questionable`.
  A pair never has more than one outstanding probe at a time; the
  sender's outstanding-probe registry is the in-process bookkeeping.
  Each tick emits at most `maxProbesPerTick` probes: when more than
  that many pairs are `Questionable`, the excess are deferred to later
  ticks and converge via the passive hop-ack timeline (122 s) instead
  of the active path. The fast active-detection figure (3 × 30 s =
  90 s to `Bad`) therefore holds only while the live `Questionable`
  set is within the per-tick budget; selection of deferred pairs is
  best-effort (map iteration order), not a stable round-robin.
- Verify-before-trust for newly observed uplinks is implemented by
  ...
  ticker picks it up on its next 30 s tick and sends the first
...
- Sending honours the Phase 0 overload gate. When the host is in
  ...
  timeline instead of via the active 90 s probe path.
...
- Probe'ы планируются sender'ом каждые `HealthProbeInterval`
  (30s) для пар в `Questionable`. Outstanding-probe registry —
  in-process бухгалтерия, не больше одного outstanding probe на пару.
  За один tick отправляется не более `maxProbesPerTick` проб: когда
  пар в `Questionable` больше, избыток откладывается на следующие
  tick'и и сходится по пассивному hop-ack таймлайну (122s) вместо
  активного. Быстрый active-detection (3 × 30s = 90s до `Bad`)
  держится только пока живой `Questionable`-набор укладывается в
  per-tick бюджет; отбор отложенных пар — best-effort (порядок
  обхода map), а не стабильный round-robin.
- Verify-before-trust для впервые наблюдаемых uplink'ов реализован
  ...
  подхватит её на следующем 30s tick'е и тогда отправит первую
...
- Send уважает Phase 0 overload gate. При overload пропускается
  ...
  passive 122s timeline вместо активного 90s probe-пути.
