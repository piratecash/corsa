...
- **Send (Tier 2, route-table fallback)** — `Router.collectRouteCandidates`
  ...
  `ProtocolVersion` (the normalized ranking key, clamped to 0 by the
  inflated-version defence in `trustedFileRouteVersion`). Filtering on
  RAW preserves the soft-demote invariant: a peer that reported
  `version > config.ProtocolVersion` keeps `RawProtocolVersion >= 12`
  so the candidate is admitted as last-resort, while its
  `ProtocolVersion == 0` pushes it to the bottom of the
  protocolVersion-DESC sort. Filtering on `ProtocolVersion` would
  collapse "inflated and clamped" with "unknown / not observed" and
  drop both — see `TestExplainFileRouteRanksInflatedVersionPeerLast`
...
in the ranking key loses the primary key under DESC ordering and
sinks to the bottom of the plan; the next paragraph describes the
single shape in which `protocol_version == 0` can still appear after
the cutover.
...
before any clamp), not by the normalized `protocol_version` ranking
...
`explainFileRoute` output. The only way `protocol_version == 0`
shows up in the plan now is the **clamped inflated** case described
in [Inflated-version defence](#inflated-version-defence): the peer
reported `version > config.ProtocolVersion`, the node-side helper
clamped the ranking key to `0`, but `RawProtocolVersion >=
FileCommandMinPeerProtocolVersion` so the candidate survives
eligibility as last-resort.
...
the connection that backs a candidate, it clamps the **ranking** value
(`PeerRouteMeta.ProtocolVersion`) down to `0` whenever
`peer_version > local_version`, while keeping the **eligibility**
value (`PeerRouteMeta.RawProtocolVersion`) at whatever the peer
actually reported. The split is what lets the inflated case stay
eligible — `RawProtocolVersion >= FileCommandMinPeerProtocolVersion`
passes the cutover filter, the candidate is admitted to the plan as
last-resort, and the `0` ranking value pushes it to the bottom of the
`protocol_version` DESC sort. After the cutover, this is the **only**
shape in which `ProtocolVersion == 0` reaches `routeCandidate`:
unknown / pre-handshake / capability-only / known-pre-cutover peers
are filtered earlier and never become candidates. Operators reading
`explainFileRoute` therefore see `ProtocolVersion == 0` as a single,
unambiguous signal — "this peer reported a version higher than our
build expects" — and the WARN log from `trustedFileRouteVersion`
disambiguates the original reported value if needed. The candidate
stays eligible — we want to deliver bytes when no trusted peer is
reachable, just not to *prefer* the suspicious one.
...
[
  {
    ...
    "protocol_version": 12,                  // normalized ranking key for this next-hop —
                                             ...
                                             // defence when the peer reported
                                             // v > config.ProtocolVersion. The raw negotiated
                                             // value lives only inside the eligibility layer
                                             // (PeerRouteMeta.RawProtocolVersion) and is not
                                             // serialized. Pre-cutover (< FileCommandMinPeerProtocolVersion)
                                             // and unknown peers are filtered out before the
                                             // plan, so the only way 0 appears here is the
                                             // clamped-inflated case from
...
sender mapping stays in `serving` indefinitely — permanently consuming
one of the 16 concurrent serving slots.
...
`protocol_version == 0` в ranking-ключе проигрывает первому ключу
при DESC и тонет в самый низ плана; следующий параграф описывает
единственную форму, в которой `protocol_version == 0` ещё может
появиться после cutover'а.
...
любого clamp), не нормализованным `protocol_version`-ranking-ключом.
...
появляются. Единственный способ, которым `protocol_version == 0`
теперь оказывается в плане, — **clamped inflated** случай из
[Защиты от завышенной версии](#защита-от-завышенной-версии): пир
заявил `version > config.ProtocolVersion`, node-side helper клампит
ranking-ключ в `0`, но `RawProtocolVersion >=
FileCommandMinPeerProtocolVersion` сохраняет кандидата как
last-resort.
...
соединение, лежащее под кандидатом, он клампит **ранжирующее**
значение (`PeerRouteMeta.ProtocolVersion`) в `0`, если
`peer_version > local_version`, а **eligibility**-значение
(`PeerRouteMeta.RawProtocolVersion`) сохраняет в том виде, в каком
пир его заявил. Это разделение позволяет inflated-кейсу остаться
пригодным — `RawProtocolVersion >= FileCommandMinPeerProtocolVersion`
проходит cutover-фильтр, кандидат попадает в план как last-resort, а
`0` в ranking-ключе толкает его в самый низ DESC-сортировки по
`protocol_version`. После cutover'а это **единственная** форма, в
которой `ProtocolVersion == 0` достигает `routeCandidate`: unknown /
pre-handshake / capability-only / known-pre-cutover peers
отфильтровываются раньше и в кандидатах не появляются. Поэтому
оператор, читая `explainFileRoute`, видит `ProtocolVersion == 0` как
однозначный сигнал — «этот пир заявил версию выше, чем понимает наш
билд», — а WARN-лог из `trustedFileRouteVersion` восстанавливает
оригинально заявленное значение, если оно нужно. Кандидат остаётся
пригодным — мы хотим иметь возможность доставить байты, когда других
пиров нет, просто не предпочитать подозрительного.
...
[
  {
    "next_hop": "<peer identity>",
    "hops": 1,
    "protocol_version": 12,                  // нормализованный ranking-ключ next-hop'а — для
                                             // легитимных пиров совпадает с raw negotiated
                                             // версией, но защита от завышенной версии клампит
                                             // ranking-ключ в 0, если пир заявил
                                             // v > config.ProtocolVersion. Raw negotiated
                                             // значение остаётся только внутри eligibility-слоя
                                             // (PeerRouteMeta.RawProtocolVersion) и наружу не
                                             // сериализуется. Pre-cutover
                                             // (< FileCommandMinPeerProtocolVersion) и unknown
                                             // пиры отфильтрованы до попадания в план, поэтому
                                             // единственный способ увидеть здесь 0 —
                                             // clamped-inflated случай из
...
`serving` навсегда — занимая один из 16 слотов параллельной отправки.
