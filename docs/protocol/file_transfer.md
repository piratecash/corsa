...
DM is **not** gated — it travels through the standard DM pipeline, which
has no capability check.

The capability gate applies to the **file command channel only**: the
sender verifies `file_transfer_v1` on the recipient (or the route's
next-hop) before emitting `chunk_request`, `chunk_response`,
`file_downloaded`, or `file_downloaded_ack`. Legacy nodes therefore never
see FileCommandFrame traffic, but they may receive a `file_announce` DM.
The `json:"command,omitempty"` tag ensures the `Command` field is absent
in normal text DMs, so legacy nodes' JSON unmarshaling is unaffected —
the announce is silently ignored.
