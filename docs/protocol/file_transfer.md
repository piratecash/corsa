...
File transfer participation is gated by capability negotiation:
`file_transfer_v1` must be advertised in the hello/welcome handshake by
both sides. The `MinimumProtocolVersion` floor enforced at handshake
already keeps any peer that predates the self-contained authenticity
wire format (`SrcPubKey`) out of the session pool, so capability is the
only gate this build needs above that floor.

The capability check applies on send and receive:

- **Send (Tier 1, direct session)** — `peerSendableConnectionsLocked` in
  `internal/core/node/service.go` filters peer sessions and drops any
  session that does not advertise `file_transfer_v1`.
- **Send (Tier 2, route-table fallback)** — `Router.collectRouteCandidates`
  in `internal/core/service/filerouter/router.go` selects next-hops
  from the routing snapshot and ranks them with `PeerRouteMeta`.
  `PeerRouteMeta` carries two version fields: `ProtocolVersion` (the
  normalized ranking key, capped at `config.ProtocolVersion` by the
  inflated-version defence in `trustedFileRouteVersion`) and
  `RawProtocolVersion` (the wire-level reported value, kept for audit
  logging and protocol-aware diagnostics). They are equal in the common
  case; they differ only when the peer reported
  `version > config.ProtocolVersion`, in which case the cap collapses
  the inflation lie to the v=local ranking tier without zeroing the
  candidate. See `TestExplainFileRouteCapsInflatedVersionAtLocal` and
  `TestFileTransferPeerRouteMetaCapsNewerVersionAtLocal` for the
  regressions that pin the cap behaviour.
- **Receive (dispatch)** — the `file_command` branch of
  `dispatchNetworkFrame` (`service.go`) rejects the frame when the
  inbound connection does not advertise `file_transfer_v1`.

The `file_announce` DM is **not** gated by `file_transfer_v1` — it
travels through the standard DM pipeline, which has no capability
check. The `json:"command,omitempty"` tag ensures the `Command` field
is absent in normal text DMs, so legacy nodes' JSON unmarshaling is
unaffected — the announce is silently ignored.
...
always beats one without health data. The ranking key is bounded
above by `config.ProtocolVersion` (newer peers are capped, not
zeroed) thanks to the inflated-version cap. Below-floor peers never
reach the plan: handshake rejects them at `MinimumProtocolVersion`,
and the `PeerRouteMeta` callback in `collectRouteCandidates` resolves
through `Service.fileTransferPeerRouteMeta` →
`fileTransferPeerRouteMetaLocked` →
`peerSendableConnectionsLocked`, which gates each next-hop on
`file_transfer_v1` capability and connected non-stalled health.
A next-hop with no usable session returns `ok = false` from the
callback and the candidate is dropped before reaching the
`routeCandidate` struct, so every candidate that does reach the plan
carries a fully-negotiated, on-or-above-floor protocol version.

Inflated peers (`RawProtocolVersion > config.ProtocolVersion`) keep
`RawProtocolVersion` at the actually-reported value but their ranking
key is capped at the local version, so they tie with v=local peers
on the primary key and the secondary keys (hops, uptime) decide.
...
`peer_version > local_version`, while keeping the **wire-level**
reported value (`PeerRouteMeta.RawProtocolVersion`) at whatever the
peer actually reported. The cap collapses the inflation lie to the
same primary-key tier as a legitimate v=local peer — neither wins the
DESC sort on the lie alone — and the secondary keys (hops ASC,
connected_at ASC) decide which of the equal-version candidates is
preferred.
...
`RawProtocolVersion` stays internal: it is kept for audit logging and
protocol-aware diagnostics, not for ranking. The wire schema of
`explainFileRoute` deliberately serialises only the capped
...
[
  {
    ...
    "protocol_version": 14,                  // normalized ranking key for this next-hop —
                                             ...
                                             // negotiated value lives only on the audit-logging
                                             // surface (PeerRouteMeta.RawProtocolVersion) and is
                                             // not serialized. Bounded above by
                                             // config.ProtocolVersion — see
                                             // [Inflated-version defence](#inflated-version-defence).
                                             // Peers below the global MinimumProtocolVersion are
                                             // rejected at handshake and never reach the plan.
...
the missing-src_pubkey gate. `SrcPubKey` is part of the permanent
file_command authenticity contract: a frame arriving without it is
malformed, forged, or produced by a misconfigured peer. Dropping is
the correct outcome in all three cases regardless of the sender's
protocol version.
...
missing-src_pubkey-гейте. `SrcPubKey` — часть постоянного контракта
file_command authenticity: кадр без него либо malformed, либо
подделанный, либо выпущен неправильно сконфигурированной нодой;
дропать — правильный результат в любом из этих случаев независимо от
версии отправителя.
...
uptime всегда выигрывал у пира без health-данных. Ranking-ключ
ограничен сверху значением `config.ProtocolVersion` благодаря cap'у
inflated-версии (более новые пиры теперь капятся, а не обнуляются).
Below-floor пиры в план не попадают: handshake отклоняет их на
`MinimumProtocolVersion`, а `PeerRouteMeta`-callback в
`collectRouteCandidates` идёт через `Service.fileTransferPeerRouteMeta`
→ `fileTransferPeerRouteMetaLocked` → `peerSendableConnectionsLocked`,
который фильтрует каждый next-hop по capability `file_transfer_v1` и
connected non-stalled health. Если пригодной сессии нет, callback
возвращает `ok = false` и кандидат отбрасывается до построения
`routeCandidate`-структуры, так что каждый кандидат, доходящий до
плана, несёт полностью согласованную, не ниже floor'а версию
протокола.

Inflated-пиры (`RawProtocolVersion > config.ProtocolVersion`)
сохраняют `RawProtocolVersion` равным заявленному значению, но их
ranking-ключ капится на локальной версии, поэтому они тайются с
v=local пирами по primary key и вторичные ключи (hops, uptime)
решают.
...
если `peer_version > local_version`, а **wire-level**-значение
...
`RawProtocolVersion` остаётся внутренним: его держат для audit-лога
и протоколо-зависимой диагностики, в ranking он не участвует.
...
[
  {
    "next_hop": "<peer identity>",
    "hops": 1,
    "protocol_version": 14,                  // нормализованный ranking-ключ next-hop'а —
                                             ...
                                             // negotiated значение остаётся только на
                                             // audit-лог-поверхности (PeerRouteMeta.RawProtocolVersion)
                                             // и наружу не сериализуется. Поле ограничено
                                             // сверху config.ProtocolVersion — см.
                                             // [Защиты от завышенной версии](#защита-от-завышенной-версии).
                                             // Пиры ниже глобального MinimumProtocolVersion
                                             // отвергаются на handshake и в план не попадают.
