...
```json
{
  ...
  "code": "peer-banned",
  "status": "closing",
  "error": "peer banned for incompatible protocol version",
  "details": { "until": "2026-05-08T12:00:00Z", "reason": "peer-ban" }
}
```
...
below. The historical `observed-address-mismatch` notice was removed in
the v12 cleanup phase; current builds emit only `peer-banned` notices
on this transport-control frame type.
...
| `frame-too-large` | Inbound frame line exceeds transport size limit | 413 Payload Too Large | A single inbound JSON command line exceeded `maxCommandLineBytes` (128 KiB) on the server's TCP reader. Connection is closed after this error. Standard protocol commands (including `relay_message` with a 64 KiB body) fit within this limit; oversized frames typically indicate a misbehaving or malicious client. Note: peer-session and handshake reads use a separate, larger limit (`maxResponseLineBytes`, 8 MiB) because response frames can contain multiple messages. **Send-path note:** the writer-side `MarshalFrameLineWithLimit` enforces a contextual budget — `protocol.MaxFrameLine` (128 KiB) for command-plane writes (`writeFrameToInboundConn`) and `protocol.MaxResponseLine` (8 MiB) for peer-session writes (`peerSessionRequest`) — and returns `ErrFrameTooLarge` to the caller. Send-path callers drop the offending frame and continue the session; they do NOT trigger `markPeerDisconnected`, because closing a peer over a self-built oversize frame would only restart the same frame on the next session and produce a reconnect storm. The announce plane additionally pre-shards via `chunkAnnounceEntriesBySize` against the 128 KiB ceiling, dropping individual oversize entries with the `announce_routes_entry_oversize_dropped` log line |
...
| `self-identity` | Handshake counterpart advertised the local node's own Ed25519 address | 409 Conflict | Surfaced when `hello.address` on the inbound path or `welcome.address` on the outbound path equals the local node identity. Raised on both directions so the node never signs an `auth_session` against its own key material, never ingests its own contact details, and never promotes its own listen address as a dial candidate. The inbound side answers with `connection_notice{code=peer-banned, reason=self-identity}` and closes the connection through the NetCore wrapper; the outbound side collapses the handshake and applies an address-level cooldown (see `connection_notice` → `reason=self-identity`) |
...
- `self-identity` - Remote hello/welcome carried the local node's own Ed25519 address (self-loopback)
...
|------|-----------|---------|
| `peer-banned` | Emitted on two independent tracks that differ by blast radius. Track 1 — **per-peer storm-suppression signal** — fires immediately on the first incompatible hello (inside the inbound `hello` handler, before teardown): the responder tells the initiator that THIS specific peer address is banned for the incompatibility window so the dialler-side gate stops fanning out reconnects, while compatible siblings behind a shared egress IP (NAT / VPN / Tor exit / multi-homed host) stay reachable. Track 2 — **transport-level IP blacklist** — fires on the session that tripped the IP-level accumulation inside the responder's `addBanScore` transition, after repeated abuse from the same IP. In both tracks the notice rides out on the still-open ConnID — the responder never opens a fresh socket just to deliver it. After a track-2 transition, subsequent raw reconnects from the blacklisted IP are closed silently; after a track-1 emission only this specific peer is suppressed on the dialler side | `details.until` (UTC RFC3339) is the wall-clock expiration of the ban. `details.reason` distinguishes the tracks: `peer-ban` is emitted on first contact for per-peer incompatibility and MUST be scoped to the peer address, not the IP; `blacklisted` is emitted when the IP-wide threshold is crossed and applies to every peer behind that IP; `self-identity` is emitted by the inbound hello handler when the remote claims the local node's own Ed25519 address — the dialler MUST treat this endpoint as a self-loopback dead-end and apply an address-level cooldown (no sibling propagation: self-identity collisions are a property of a specific dial target, not its IP). Initiators MUST accept all reason values and record the window unchanged — a responder rollout does not require a concurrent client rollout, and scoping the persistence correctly (per peer vs per IP) is the initiator's responsibility based on `reason`. The initiator scopes persistence by `reason`. `peer-ban` / `self-identity` write ONLY a per-peer record against the persisted peer entry so the dial gate skips THIS specific address while sibling peers behind the same egress IP stay dialable. `blacklisted` writes a per-peer record for the offending address too AND counts it toward an IP-wide escalation: the IP-wide record (keyed on the server IP, host part of host:port — suppressing every sibling PeerAddress behind that IP, including peers discovered later via peer exchange) is written ONLY once a configurable number of DISTINCT, non-expired offenders behind the same IP have each sent `blacklisted`, and its expiry is the threshold-th largest live offender window so it lapses once too few offenders remain. A single `blacklisted` notice therefore suppresses only the offending peer — an initiator MUST NOT undial innocent siblings (NAT / VPN / Tor-exit neighbours) from one notice. The IP-wide record is persisted separately from the per-peer record and survives restart so the retry storm the notice was designed to end cannot resume from disk; the pre-escalation offender counter is volatile (not persisted). No sender-side mirror is written: the dial-gate check consults both tables on every candidate, so a record in either one is enough to suppress. A successful handshake to any peer on the affected IP later clears TWO scopes atomically: (1) the handshaking peer's own per-peer record (unconditional, regardless of reason — direct proof this address accepts us; a no-op when no row exists), and (2) the IP-wide record keyed on the canonical host of the dialled address (one sibling's success speaks for every sibling behind that egress). Both clears flush to disk together. Per-peer rows with `reason=peer-ban` or `reason=self-identity` on OTHER sibling addresses are intentionally NOT cleared by the IP-wide recovery: those are standalone responder decisions on specific addresses and a handshake with a sibling is not proof the responder has forgiven them. Only the per-peer and IP-wide records persist, so recovery has at most those two scopes to unwind; the volatile offender counter is dropped as a side effect of clearing the IP-wide scope |
...
«Коды connection_notice». Исторический `observed-address-mismatch`
notice удалён в фазе v12 cleanup; текущие сборки эмитят на этом
transport-control фрейме только `peer-banned`.
...
| `frame-too-large` | Входящая строка команды превышает транспортный лимит размера | 413 Payload Too Large | Одна входящая строка JSON-команды превысила `maxCommandLineBytes` (128 KiB) на TCP-ридере сервера. Соединение закрывается после этой ошибки. Стандартные команды протокола (включая `relay_message` с телом 64 KiB) укладываются в этот лимит; слишком большие кадры обычно указывают на некорректный или вредоносный клиент. Примечание: peer-session и handshake reads используют отдельный, больший лимит (`maxResponseLineBytes`, 8 MiB), поскольку ответы могут содержать множество сообщений. **Send-path note:** writer-side `MarshalFrameLineWithLimit` применяет контекстуальный бюджет — `protocol.MaxFrameLine` (128 KiB) для command-плоскости (`writeFrameToInboundConn`) и `protocol.MaxResponseLine` (8 MiB) для peer-session writes (`peerSessionRequest`) — и возвращает `ErrFrameTooLarge` вызывателю. Send-path вызыватели сбрасывают такой фрейм и продолжают сессию; `markPeerDisconnected` НЕ вызывается, иначе закрытие peer'а из-за нашего же oversize-фрейма перезапустило бы тот же фрейм в следующей сессии и спровоцировало бы шторм reconnect'ов. Announce-плоскость дополнительно пред-шардирует через `chunkAnnounceEntriesBySize` против 128 KiB потолка, отбрасывая отдельные oversize-записи с лог-строкой `announce_routes_entry_oversize_dropped` |
...
| `self-identity` | Контрагент рукопожатия объявил Ed25519-адрес самой локальной ноды | 409 Conflict | Возникает, когда `hello.address` на входящей стороне или `welcome.address` на исходящей стороне совпадает с identity локальной ноды. Срабатывает в обоих направлениях, чтобы нода никогда не подписывала `auth_session` против своего же ключевого материала, никогда не импортировала собственные контактные данные и никогда не повышала собственный listen-адрес как dial-кандидат. Входящая сторона отвечает `connection_notice{code=peer-banned, reason=self-identity}` и закрывает соединение через NetCore-обёртку; исходящая сторона схлопывает handshake и применяет address-level cooldown (см. `connection_notice` → `reason=self-identity`) |
...
- `self-identity` - Удалённый hello/welcome принёс собственный Ed25519-адрес локальной ноды (self-loopback)
...
|-----|---------------------|--------|
| `peer-banned` | Эмитится по двум независимым трекам, различающимся радиусом поражения. Трек 1 — **per-peer signal подавления шторма** — срабатывает немедленно на первой несовместимой hello (внутри обработчика входящего `hello`, до teardown): ответчик сообщает инициатору, что ИМЕННО ЭТОТ peer-адрес забанен на окно несовместимости, чтобы dialler-side gate перестал fan-outить реконнекты, тогда как совместимые siblings за общим egress IP (NAT / VPN / Tor exit / multi-homed host) остаются достижимыми. Трек 2 — **transport-level IP blacklist** — срабатывает на сессии, которая переключила IP-level накопление внутри транзита `addBanScore` у ответчика, после повторяющегося abuse с того же IP. В обоих треках уведомление уходит по ещё живому ConnID — ответчик не открывает новый сокет специально ради доставки. После перехода трек-2 последующие raw-переподключения с чёрного IP закрываются молча; после эмиссии трек-1 подавляется только этот конкретный peer на dialler-side | `details.until` (UTC RFC3339) — момент истечения бана по стенным часам. `details.reason` различает треки: `peer-ban` эмитится на первом контакте для per-peer несовместимости и ДОЛЖЕН быть scoped к peer-адресу, а не к IP; `blacklisted` эмитится при пересечении IP-wide порога и применяется ко всем peer'ам за этим IP; `self-identity` эмитится обработчиком входящего hello, когда удалённая сторона объявила собственный Ed25519-адрес локальной ноды — инициатор ДОЛЖЕН рассматривать этот endpoint как self-loopback dead-end и применить address-level cooldown (без sibling propagation: self-identity коллизии — свойство конкретного dial-таргета, а не его IP). Инициатор ДОЛЖЕН принимать все значения reason и записывать окно без изменений — раскатка ответчика не требует одновременной раскатки клиентов, и корректный scoping персистенции (per peer vs per IP) — ответственность инициатора на основе `reason`. Инициатор скоупит персистенцию по `reason`. `peer-ban` / `self-identity` пишут ТОЛЬКО per-peer запись в persisted-мету пира, чтобы гейт дайлера пропускал ИМЕННО этот адрес, а sibling-пиры за тем же egress IP остаются дайлебельными. `blacklisted` пишет per-peer запись offender'а ТОЖЕ И засчитывает её в IP-wide эскалацию: IP-wide запись (ключённая на серверном IP, host-часть host:port — подавляет каждый sibling-PeerAddress за этим IP, включая обнаруженных позже через peer exchange) пишется ТОЛЬКО после того как заданное число РАЗЛИЧНЫХ непросроченных offender'ов за тем же IP пришлют `blacklisted`, а её срок — это k-я по величине живая offender-window, так что она спадает, когда offender'ов остаётся слишком мало. Одиночный `blacklisted` подавляет только самого offender'а — инициатор НЕ ДОЛЖЕН отключать невиновных siblings (NAT / VPN / Tor-exit соседей) по одному уведомлению. IP-wide запись персистится отдельно от per-peer записи и переживает рестарт, чтобы retry-storm не возобновился с диска; pre-escalation счётчик offender'ов волатилен (не персистится). Зеркала на стороне sender не пишутся: проверка дайл-гейта на каждом кандидате консультирует обе таблицы, и записи в любой из них достаточно для подавления. Успешное рукопожатие с ЛЮБЫМ пиром на затронутом IP позже атомарно очищает ДВА scope: (1) собственную per-peer запись handshaking-пира (безусловно, независимо от reason — прямое доказательство, что этот адрес принимает нас; no-op, если записи нет), и (2) IP-wide запись, ключённую на каноническом host дайленного адреса (успех одного sibling говорит за всех siblings за этим egress). Обе очистки сбрасываются на диск вместе. Per-peer записи с `reason=peer-ban` или `reason=self-identity` на ДРУГИХ sibling-адресах намеренно НЕ очищаются IP-wide recovery: это автономные решения ответчика на конкретных адресах, и рукопожатие с sibling не доказывает, что ответчик их простил. Персистятся только per-peer и IP-wide записи, поэтому при восстановлении нужно размотать максимум эти два scope; волатильный счётчик offender'ов сбрасывается как побочный эффект очистки IP-wide scope |
