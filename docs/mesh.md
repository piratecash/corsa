...
listen address, and node type at `INFO` level with message
`node_service_starting`. When an inbound hello arrives from a
...
sequenceDiagram
    ...
    Note over A,B: Phase 3 — Initial Sync (one-time, get_peers is conditional)
...
> **Inbox subscription is folded into authentication.** `auth_session` proves
> the initiator's identity, and the responder installs the subscription
> (`registerHelloRoute`) and replays the backlog (`pushBacklogToSubscriber`)
> at auth. Every node receives its own inbox over ITS OWN outbound sessions
> (where it is the authenticating initiator), and gossip guarantees mesh-wide
> propagation. The legacy `subscribe_inbox` / `subscribed` / `request_inbox`
> commands were removed when `MinimumProtocolVersion` reached 20.
...
- **Peer announcement** — when a new peer authenticates on an inbound
  ...
  sessions via `announce_peer`. **Trust contract:** the address we
  gossip is the **observed TCP source host** combined with the
  **self-reported `advertise_port`** — never the claimed `hello.Listen`
  host verbatim, and never the inbound TCP source port. A peer can lie
  about its listen IP (stale DDNS, misconfig, or deliberately), but it
  cannot forge the IP the packets actually arrive from; the TCP source
  port, however, is an ephemeral NAT mapping and is unusable for
  dial-back, so the port is taken from `hello.advertise_port` (with
  fallback to `config.DefaultPeerPort` on absent or out-of-range
  values). Both halves are passed through `normalizePeerAddress` so
  the announce path shares the exact same trust rules as the learn
  path (`learnPeerFromFrame`). Under the v12 wire contract `hello.Listen`
  is empty, and `observedAnnounceAddressFromHello` synthesises the
  advertised endpoint from the observed host plus `advertise_port`
  before handing it to `normalizePeerAddress`. When the helper returns
  `false` (listener disabled, empty observed, malformed advertised,
  or any forbidden range) no announcement is emitted. The frame
  includes the peer's `node_type` ("full" or "client"), but
  recipients treat it as
...
5. **Push and gossip are independent.** For addressed DMs the node takes
   ...
   propagation. Push is an optimization for latency; gossip spreads the
   message so a relay with a live route to the recipient can forward it.
   Relays buffer transit envelopes only in-flight (transit window) — they
   are not mailboxes; delivery beyond the window is the sender's
   end-to-end retry.
6. The message stays in the SENDER's own `s.topics` (local messages are
   not swept), so the sender's node can keep re-attempting delivery; a
   recipient reconnecting to the sender's node also receives it via
   backlog replay there.
...
- Held **in memory only** — relay/queue state is **not** persisted and does
  **not** survive a process restart. Recovery after a restart is sender-side
  end-to-end retry: the delivery retry scheduler (delivery_retry.go)
  re-sends until the delivered/seen receipt arrives, reseeded from chatlog
  after a restart.
...
flowchart TD
    subgraph "Success responses (dispatchNetworkFrame)"
        CMD["Any command:<br/>hello, ping, get_peers,<br/>push_message, announce_peer, ..."]
...
listen address и тип ноды на уровне `INFO` с сообщением
...
sequenceDiagram
    ...
    Note over A,B: Фаза 3 — Начальная синхронизация (однократно, get_peers условно)
...
> **Подписка на inbox свёрнута в аутентификацию.** `auth_session` доказывает
> identity инициатора, а отвечающая нода на auth ставит подписку
> (`registerHelloRoute`) и реплеит backlog (`pushBacklogToSubscriber`).
> Каждый узел получает свой inbox через СОБСТВЕННЫЕ исходящие сессии (где он
> — аутентифицирующийся инициатор), а gossip гарантирует распространение по
> mesh. Легаси-команды `subscribe_inbox` / `subscribed` / `request_inbox`
> удалены, когда `MinimumProtocolVersion` достиг 20.
...
- **Анонс peer'ов** — когда новый peer аутентифицируется через входящее
  ...
  анонсируем **наблюдаемый TCP source host** в паре с **самозаявленным
  `advertise_port`** — никогда не заявленный в `hello.Listen` хост
  дословно и никогда не входящий TCP source port. Пир может соврать
  про свой listen IP (устаревший DDNS, ошибка конфига или умышленно),
  но не может подделать IP, с которого реально приходят пакеты; TCP
  source port, напротив, эфемерное NAT-mapping и для dial-back
  непригодно, поэтому порт берём из `hello.advertise_port` (с
  фолбэком на `config.DefaultPeerPort` при отсутствии или невалидном
  значении на проводе). Обе половины проходят через
  `normalizePeerAddress`, поэтому announce-путь использует ровно те
  же правила доверия, что и learn-путь (`learnPeerFromFrame`). Под
  v12-wire `hello.Listen` пуст, и `observedAnnounceAddressFromHello`
  синтезирует advertised-эндпоинт из observed host + `advertise_port`
  до того, как передать его в `normalizePeerAddress`. Если хелпер
  возвращает `false` (listener отключён, пустой observed, некорректный
...
5. **Push и gossip независимы.** Для адресных DM нода атомарно берёт
   ...
   латентности; gossip разносит сообщение, чтобы relay с живым
   маршрутом до получателя мог его переслать. Relay-ноды буферизуют
   транзитные конверты только in-flight (транзитное окно) — они не
   mailbox; доставка за пределами окна — end-to-end retry отправителя.
6. Сообщение остаётся в `s.topics` СОБСТВЕННОЙ ноды отправителя
   (локальные сообщения не выметаются), поэтому нода отправителя может
   продолжать попытки доставки; получатель, переподключившийся к ноде
   отправителя, также получит его там через replay backlog'а.
...
- Держатся **только в памяти** — relay/queue state **не** персистится и **не**
  переживает рестарт процесса. Восстановление после рестарта — это end-to-end
  retry на стороне отправителя: scheduler ретраев доставки
  (delivery_retry.go) переотправляет до прихода delivered/seen-квитанции и
  пересеивается из chatlog после рестарта.
...
flowchart TD
    subgraph "Успешные ответы (dispatchNetworkFrame)"
        CMD["Любая команда:<br/>hello, ping, get_peers,<br/>push_message, announce_peer, ..."]
        WJF["writeJSONFrame()<br/>(async, fire-and-forget)"]
    end
