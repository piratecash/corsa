...
```json
{
  ...
  "metrics": {
    ...
    "ReverseEvents": {},
    "SendRefusals": {"missing_transit_capability": 3},
    "StartedAt": "2026-09-06T09:00:00Z",
    "ReadAt": "2026-09-06T09:31:44.512837401Z"
  },
...
| `metrics.SendRefusals` | object | **Capability-rollout telemetry** (`docs/refactoring/dht/05-rollout-metrics.md`): sends the §4.3 admission gates REFUSED, keyed by which gate spoke — `missing_endpoint_capability` (the peer never advertised `mesh_datagram_v1`, so this plane does not exist for it), `missing_transit_capability` (it speaks datagrams but will not carry other nodes' frames), `unsupported_dtype` (the destination itself does not implement the type — a fact about the type, not about the transport rollout), `other` (a refusal this build cannot attribute; present so the slots sum to every refusal instead of a gate added later vanishing). Zero-valued slots are omitted. **The unit is one refused SEND, not one rejected candidate and not a failed delivery:** a send walks several candidates and may reject most of them before handing the frame to one that passes, so counting per candidate would make refusals track the length of the candidate list. The read-only route plan and the reachability probe reach the same verdicts by design and are NOT counted — otherwise opening a diagnostic would raise the number it was opened to read. Known gap: on the transit REQUEST plane the verdict is counted here but the drop reason stays `no_candidates`, because that plane has exactly one silent outcome for a transit frame
| `metrics.StartedAt` / `metrics.ReadAt` | string\|null | The period EVERY counter in this object describes: when this counter set began accumulating, and when it was read. **Both are needed to compare two readings.** `ReadAt` gives a rate its denominator; `StartedAt` says whether the two readings belong to the same run — the counters are cumulative and in-memory, so 100 refusals before a restart and 120 after look like a difference of 20 while describing two unrelated runs, and no comparison of the counters to themselves detects that (the second run may have passed the first). A pair of readings is admissible only when `ReadAt` strictly increases AND `StartedAt` is identical; otherwise discard the pair rather than subtract. `null` means the period is unknown — never read it as the year 1. Rendered with sub-second precision for the same reason `fetchRouteSummary.mode_selection.read_at` is: two reads inside one second are exactly what somebody watching a rollout produces
...
| `metrics.SendRefusals` | object | **Телеметрия раскатки capability** (`docs/refactoring/dht/05-rollout-metrics.md`): отправки, которые ОТВЕРГЛИ гейты допуска §4.3, с разбивкой по тому, какой гейт сработал — `missing_endpoint_capability` (пир не объявлял `mesh_datagram_v1`, для него этой плоскости не существует), `missing_transit_capability` (датаграммы понимает, но чужие кадры не носит), `unsupported_dtype` (сам получатель не реализует тип — факт про тип, а не про раскатку транспорта), `other` (отказ, который сборка не смогла атрибутировать; нужен, чтобы слоты складывались во все отказы, а гейт, добавленный позже, не исчезал). Нулевые слоты опускаются. **Единица — одна отвергнутая ОТПРАВКА, а не один отбракованный кандидат и не неудачная доставка:** отправка обходит несколько кандидатов и может отбраковать почти всех перед тем, как отдать кадр прошедшему, поэтому подсчёт по кандидатам заставил бы отказы следовать длине списка. Read-only план маршрута и проба достижимости приходят к тем же вердиктам по построению и НЕ считаются — иначе открытие диагностики поднимало бы число, ради которого её открыли. Известный пробел: на транзитной REQUEST-плоскости вердикт здесь считается, но причина дропа остаётся `no_candidates`, потому что у транзитного кадра на этой плоскости ровно один тихий исход
| `metrics.StartedAt` / `metrics.ReadAt` | string\|null | Период, который описывают ВСЕ счётчики этого объекта: когда этот набор начал накапливать и когда его прочитали. **Оба нужны, чтобы сравнить два замера.** `ReadAt` даёт скорости знаменатель; `StartedAt` говорит, относятся ли замеры к одному запуску — счётчики накопительные и живут в памяти, поэтому 100 отказов до рестарта и 120 после выглядят как разность 20, описывая при этом два несвязанных запуска, и никакое сравнение счётчиков с самими собой этого не обнаружит (второй запуск мог обогнать первый). Пара замеров допустима, только если `ReadAt` строго растёт И `StartedAt` совпадает; иначе пару отбрасывают, а не вычитают. `null` означает, что период неизвестен, — читать это как первый год нельзя. Отдаётся с долями секунды по той же причине, что и `fetchRouteSummary.mode_selection.read_at`: два чтения внутри одной секунды — ровно то, что производит внимательное наблюдение за раскаткой
