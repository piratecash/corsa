...
| `ipStateMu` | `bans`, `inboundByIP`, `observedAddrs`, `observedIPHistoryByPeer`, `bannedIPSet`, `remoteBannedIPs` |
...
Трогает peer-домен (`persistedMeta` через `clearRemoteBanLocked`) и `ipStateMu` (`remoteBannedIPs` через `clearRemoteIPBanLocked`). Порядок: `peerMu.Lock` снаружи → `ipStateMu.Lock` внутри. Внутри одного внешнего `peerMu`-региона `ipStateMu.Lock/Unlock` явно оборачивает вызов `clearRemoteIPBanLocked`, потому что этот helper требует от вызывающего уже держать write-lock `ipStateMu`.
