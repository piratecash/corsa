...
- `routingSnapshot` — `routing.Table.t.mu.RLock` only (separate mutex inside the `routing` package, not covered by this split). The deep copy is gated by `routing.Table.ConsumeDirty()` (CAS true→false) so a steady-state idle node skips the rebuild entirely; the publisher rebuilds only when a writer (`UpdateRoute`, `WithdrawRoute`, `AddDirectPeer`, `RemoveDirectPeer`, `InvalidateTransitRoutes`, `TickTTL`, `RecordSuccessfulRouteAdd`) flagged the table dirty since the last consume. Cached consumers (`Service.RoutingSnapshot()` for RPC, `Service.reachableIDsFrame()` for desktop remote-RPC, the file router's `RouteSnap` callback used by transit forwarding inside `HandleInbound`) take no routing-table mutex on the read path. `routeTableHandler` additionally enriches each unique next-hop identity through `RoutingProvider.PeerTransport`, which currently takes `peerMu.RLock` — that handler therefore stays exposed to peer-domain writer storms even after the routing-table coupling was eliminated; migrating `PeerTransport` onto a cached peer-transport view is tracked separately. Three production callsites deliberately bypass the cache and read the routing table directly via `routing.Table.Lookup(peer)` — a per-destination read that takes `t.mu.RLock` for an O(K) slice scan over only that destination's routes, not for the whole table: `file_integration.isPeerReachable` (gates user-initiated file transfers), `filerouter.Router.SendFileCommand` (locally-originated send path, wired through `RouterConfig.RouteLookup` to the same Lookup), and `filerouter.Router.ExplainRoute` (diagnostic surface, must mirror the live send). All three need immediate visibility of a route accepted seconds before the call — a 500 ms-stale view would surface as "isPeerReachable says yes, SendFile fails" or as "ExplainRoute shows a path the live send cannot use". `isPeerReachable` holds `s.peerMu.RLock` only for the peer-domain probe and releases it before the routing read, so the two locks remain sequential, not nested; the file-router callsites take no Service-domain mutex at all on the locally-originated path. The transit forwarding path inside `HandleInbound` keeps using the cached `RouteSnap` because in-flight frames carry their own metadata and a one-tick delay on a freshly added route is harmless there.
