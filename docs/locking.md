...
| `deliveryMu` | `pending`, `pendingKeys`, `outbound`, `relayRetry`, `awaitingDelivered`, `awaitingSeenAck`, `seenReceipts`, `receipts`, `upstream` |
...
The delivery retry scheduler (`delivery_retry.go`) owns `awaitingDelivered` / `awaitingSeenAck`, both delivery-domain fields. `retryDueDeliveries` takes `deliveryMu.Lock` alone, snapshots the due entries (bumping their schedule in place), releases, and only then performs the sends — network I/O never runs under the mutex. The `*Locked` helpers require the caller to hold `deliveryMu.Lock`; registration call sites (`storeIncomingMessage` for own outgoing DMs, `storeDeliveryReceipt` for own seen receipts) take `deliveryMu` with no other domain mutex held, or inside the existing canonical `peerMu → deliveryMu` window of `storeDeliveryReceipt`.
...
#### `retryDueDeliveries` / `registerAwaitingDeliveredLocked` / `registerAwaitingSeenAckLocked` (sender-side e2e retry)
Scheduler ретраев доставки (`delivery_retry.go`) владеет `awaitingDelivered` / `awaitingSeenAck` — оба поля delivery-домена. `retryDueDeliveries` берёт только `deliveryMu.Lock`, снапшотит due-записи (сдвигая их расписание на месте), освобождает и только потом выполняет отправки — сетевой I/O никогда не идёт под мьютексом. Хелперы `*Locked` требуют от caller-а `deliveryMu.Lock`; точки регистрации (`storeIncomingMessage` для собственных исходящих DM, `storeDeliveryReceipt` для собственных seen-квитанций) берут `deliveryMu` без других доменных мьютексов либо внутри существующего канонического окна `peerMu → deliveryMu` в `storeDeliveryReceipt`.
...
- RPC-хэндлер `fetchRouteHealth` (в `internal/core/rpc/routing_commands.go`) синхронно зовёт `RoutingProvider.HealthSnapshot()` per request. Тот вызов приходит в `routing.Table.HealthSnapshot`, которая берёт `t.mu.RLock()`, deep-copy'ит каждый `RouteHealthState` через `healthStore.snapshotLocked`, отпускает lock и возвращает value-slice. Каждое RPC-обращение, таким образом, кратко берёт `t.mu` в R-режиме — сейчас НИКАКОГО `atomic.Pointer[healthSnapshot]` кэша между RPC и storage НЕТ. Lock window короткий (один проход по map, ≤24 entries в densely-connected продакшен-mesh'ах) и под R-режимом не блокирует других reader'ов. Writer-пути с эксклюзивным lock'ом (`MarkHopAck`, `MarkProbeAck`, `TickHealth`, health-bookkeeping в `UpdateRoute`) конкурируют с одновременными `fetchRouteHealth`-вызовами как с любым другим reader'ом, но writer-preference Go'шной `sync.RWMutex` всё ещё применяется, так что writer в очереди блокирует новые RPC-чтения.
